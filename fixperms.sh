#!/bin/sh
# sed 's/^/\"/; s/$/\"/;'    <- this will quote the line
s3cmd ls --recursive s3://manager.dockerwordpress.com | grep \\.html | cut -c 30- | sed 's/^/\"/; s/$/\"/;' | xargs s3cmd modify --add-header=Cache-Control:max-age=86400 -m text/html
